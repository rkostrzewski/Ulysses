...
namespace Ulysses.ProcessingAlgorithms.Tests
{
    ...
    public class ImageProcessingAlgorithmInterfaceTests
    {
        ...
        public void AllImageProcessingAlgorithmsShouldContainConstructorWithIImageProcessingAlgorithmTemplate()
        {
            ...
            var classesThatImplementTheInterface =
                iImageProcessingAlgorithmType.Assembly.GetTypes()
                                             .Where(t => iImageProcessingAlgorithmType.IsAssignableFrom(t))
                                             .Where(t => t.IsClass && t.IsPublic && !t.IsAbstract);

            var classesConstructorsThatTakeImplementationOfIImageProcessingAlgorithmType =
                classesThatImplementTheInterface.Select(c => c.GetConstructors()).Select(cc => cc.Where(c =>
                {
                    var constructor = c;
                    ...
                    return iImageProcessingAlgorithmTemplateType.IsAssignableFrom(parameterType) && parameterType.IsClass && parameterType.IsPublic &&
